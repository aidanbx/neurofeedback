...
def test_baseline_not_ready_initially():
    ...
    features = engine.update(make_absolute(), make_relative(), quality_score=90.0, artifact_fraction=0.0)
...
def test_baseline_ready_after_min_samples():
    engine = MetricsEngine()
    engine.set_params({"baseline_min_sec": 5.0})  # 5s / 0.5 = 10 samples needed
    for _ in range(10):
        features = engine.update(make_absolute(), make_relative(), quality_score=90.0, artifact_fraction=0.0)
...
def test_quality_gating_skips_bad_samples():
    ...
    for _ in range(20):
        engine.update(make_absolute(), make_relative(), quality_score=30.0, artifact_fraction=0.5)
    features = engine.update(make_absolute(), make_relative(), quality_score=30.0, artifact_fraction=0.5)
...
def test_smoothing_asymmetric():
    ...
    # Fill baseline
    for _ in range(5):
        engine.update(make_absolute(alpha=1.0), make_relative(), quality_score=90.0, artifact_fraction=0.0)
    # Step up
    f_up = engine.update(make_absolute(alpha=10.0), make_relative(), quality_score=90.0, artifact_fraction=0.0)
    s_up = f_up["Alpha"].smoothed
    # Reset and step down
    engine2 = MetricsEngine()
    engine2.set_params({"baseline_min_sec": 1.0, "rise_alpha": 0.5, "fall_alpha": 0.1})
    for _ in range(5):
        engine2.update(make_absolute(alpha=10.0), make_relative(), quality_score=90.0, artifact_fraction=0.0)
    f_down = engine2.update(make_absolute(alpha=1.0), make_relative(), quality_score=90.0, artifact_fraction=0.0)
...
def test_reset_baseline():
    engine = MetricsEngine()
    for _ in range(10):
        engine.update(make_absolute(), make_relative(), quality_score=90.0, artifact_fraction=0.0)
